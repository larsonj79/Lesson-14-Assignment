...
ttncpercs <- ttnc %>% 
  summarize(percsurv = mean(Survived),
            percmale = mean(Sex == "male"),
            percfemale = mean(Sex == "female"),
            percS = mean(Embarked == "S"),
            percC = mean(Embarked == "C"),
            percQ = mean(Embarked == "Q"))
...
meanage <- mean(ttnc$Age[!is.na(ttnc$Age)])
...
ttnc$Pclass <- factor(ttnc$Pclass)
ttnc$Sex <- factor(ttnc$Sex)
ttnc$Embarked <- factor(ttnc$Embarked)
...
mod1 <- lm(Age ~ Pclass + Sex + SibSp + Parch + Embarked, data = ttnc)
summary(mod1)
...
agepred <- predict(mod1, newdata = ttnc[is.na(ttnc$Age),])
...
ttnc2 <- ttnc
ttnc2$Age[is.na(ttnc2$Age)] <- agepred
...
mod2 <- glm(Survived ~ Sex, data = ttnc2, family = "binomial")
summary(mod2)
...
# female.
fempredprob <- exp(fempred) / (1 + exp(fempred))
  
#15. Use *malepred* to calculatte *malepredprob*, the survival probability for a
# male.
malepredprob <- exp(malepred) / (1 + exp(malepred))
...
mod5 <- glm(Survived ~ Sex + Pclass + Sex:Pclass, data = ttnc2, family = "binomial")
summary(mod5)

#23. According to *mod5*, what is the probability of survival of a male 
# traveling in Third Class? Save it as mtcpredprob2.
mtcpred <- mod5$coefficients[1] + mod5$coefficients[2] + mod5$coefficients[4] + mod5$coefficients[6] 
mtcpredprob2 <- exp(mtcpred) / (1 + exp(mtcpred))

#24. What is the probability of survival of a female traveling in Third Class?
# Save it as ftcpredprob2.
ftcpred <- mod5$coefficients[1] + mod5$coefficients[4]
ftcpredprob2 <- exp(ftcpred) / (1 + exp(ftcpred))
...
ttncsurvprop <- ttnc2 %>% 
  group_by(Sex, Pclass) %>% 
  summarize(PropSurv = mean(Survived))
...
mod6 <- glm(Survived ~ Sex + Pclass + Age + Sex:Pclass + Sex:Age, data = ttnc2, family = "binomial")
summary(mod6)
...
survpredbin <- ifelse(survpred > 0, 1, 0)

#35. Create the confusion matrix table and save it as *confmat*. Print 
# it to the screen.
confmat <- table(ttnc2$Survived, survpredbin)
...
ttnctest$Pclass <- factor(ttnctest$Pclass)
ttnctest$Sex <- factor(ttnctest$Sex)
ttnctest$Embarked <- factor(ttnctest$Embarked)

ttncpred <- predict(mod6, newdata = ttnctest)
ttncpredbin <- ifelse(ttncpred > 0, 1, 0)
ttncpredbin <- data.frame(ttncpredbin)
names(ttncpredbin) <- "pred"
...
write_csv(ttncpredbin, "ttncpredout.csv")
